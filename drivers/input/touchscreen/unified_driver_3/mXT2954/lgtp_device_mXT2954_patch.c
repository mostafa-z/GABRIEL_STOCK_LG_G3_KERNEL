...
#define LGTP_MODULE "[mXT2954-Patch]"
...
static void mxt_patch_calibration(struct mxt2954_ts_data *data)
{
	mxt_write_object(data, MXT_GEN_COMMAND_T6, MXT_COMMAND_CALIBRATE, 1);
}
int mxt_read_mem(struct mxt2954_ts_data *data, u16 reg, u8 len, void *buf)
{
	int ret = 0;

	ret = Mxt2954_I2C_Read(data->client, reg, 2, buf, len);

	return ret;
}

int mxt_write_mem(struct mxt2954_ts_data *data, u16 reg, u8 len, u8 *buf)
{
	int ret = 0;

	ret = Mxt2954_I2C_Write(data->client, reg, buf, len);

	return ret;
}

static int mxt_patch_start_timer(struct mxt2954_ts_data *data, u16 period)
{
...
static int mxt_patch_stop_timer(struct mxt2954_ts_data *data)
{
	struct mxt2954_object *object = NULL;
	int ret = 0;
...
static int mxt_patch_write_stage_cfg(struct mxt2954_ts_data *data, struct stage_cfg *pscfg, bool do_action)
{
	if (do_action) {
...
static int mxt_patch_write_action_cfg(struct mxt2954_ts_data *data, struct action_cfg *pacfg, bool do_action)
{
	if (do_action) {
...
static int mxt_patch_write_trigger_cfg(struct mxt2954_ts_data *data, struct trigger_cfg *ptcfg, bool do_action)
{
	if (do_action) {
...
static int mxt_patch_write_event_cfg(struct mxt2954_ts_data *data, struct event_cfg *pecfg, bool do_action)
{
	if (do_action) {
...
static int mxt_patch_predefined_action(struct mxt2954_ts_data *data, u8 action_id, u16 action_val, bool do_action)
{
	if (do_action) {
		switch (action_id) {
...
static void mxt_patch_init_tpos(struct mxt2954_ts_data *data, struct touch_pos *tpos)
{
	int i = 0;
...
static bool mxt_patch_check_locked(struct mxt2954_ts_data *data, struct touch_pos *tpos, u8 tid, u16 x, u16 y)
{
	s16 diffx = 0, diffy = 0;
	u32 distance = 0;
...
static void mxt_patch_check_pattern(struct mxt2954_ts_data *data, struct touch_pos *tpos, u8 tid, u16 x, u16 y, u8 finger_cnt)
{
...
static void mxt_patch_init_supp(struct mxt2954_ts_data *data, struct touch_supp *tsup)
{
	tsup->old_time = jiffies_to_msecs(jiffies);
	tsup->repeat_cnt = 0;
}

static void mxt_patch_check_supp(struct mxt2954_ts_data *data, struct touch_supp *tsup)
{
...
static int mxt_patch_item_lval(struct mxt2954_ts_data *data, u16 *psrc_item, u8 src_id)
{
	if (psrc_item != NULL) {
		if (MXT_PATCH_ITEM_NONE <= src_id && src_id < MXT_PATCH_ITEM_END)
...
static int mxt_patch_item_rval(struct mxt2954_ts_data *data, u16 *psrc_item, struct item_val ival)
{
...
static int mxt_patch_item_check(struct mxt2954_ts_data *data, u16 *psrc_item, struct test_item *ptitem, bool do_action)
{
...
static int mxt_patch_stage_timer(struct mxt2954_ts_data *data, u16 period, bool do_action)
{
	int ret = 0;
	u32 time = period * 10;
...
static int mxt_patch_parse_test_line(struct mxt2954_ts_data *data, u8 *ppatch, u16 *psrc_item, u16 *check_cnt, bool do_action)
{
...
static int mxt_patch_parse_stage(struct mxt2954_ts_data *data, u8 *ppatch, u16 *ptline_addr, u8 *ptline_cnt, bool do_action)
{
...
static u16 mxt_patch_match_lval(struct mxt2954_ts_data *data, u8 *pmsg, u8 offset, u16 mask)
{
	u16 lval = 0;
...
static int mxt_patch_match_check(struct mxt2954_ts_data *data, u8 *pmsg, struct match *pmatch, bool do_action)
{
...
static int mxt_patch_trigger_check(struct mxt2954_ts_data *data, u8 object, u8 index, u8 *pmsg)
{
	u8 reportid = pmsg[0];
	u8 type = 0, id = 0;
...
static int mxt_patch_parse_trigger(struct mxt2954_ts_data *data, u8 *ppatch, u8 *pmsg, bool do_action, u8 option)
{
...
int mxt_patch_parse_event(struct mxt2954_ts_data *data, u8 *ppatch, bool do_action)
{
...
static int mxt_patch_parse_header(struct mxt2954_ts_data *data, u8 *ppatch, u16 *pstage_addr, u16 *ptrigger_addr, u16 *pevent_addr)
{
...
static int mxt_patch_test_source(struct mxt2954_ts_data *data, u16 *psrc_item)
{
	int i = 0;
	u8 *ppatch = data->patch.patch;
	u16 *pstage_addr = data->patch.stage_addr;
...
static int mxt_patch_make_source(struct mxt2954_ts_data *data, struct test_src *tsrc)
{
	if (tsrc->charger >= 0)
...
static int mxt_patch_start_stage(struct mxt2954_ts_data *data)
{
	if (data->patch.patch) {
...
static int mxt_patch_test_trigger(struct mxt2954_ts_data *data, struct mxt2954_message *message, u8 option)
{
	int i = 0;
	u8 *ppatch = data->patch.patch;
...
static void mxt_patch_T6_object(struct mxt2954_ts_data *data, struct mxt2954_message *message)
{
	/*Normal mode */
...
static void mxt_patch_T9_object(struct mxt2954_ts_data *data, struct mxt2954_message *message)
{
	int id = 0;
...
static void mxt_patch_T15_object(struct mxt2954_ts_data *data, struct mxt2954_message *message)
{
	struct test_src tsrc = {0};
...
static u8 check_pattern_tracking_condition(struct mxt2954_ts_data *data, struct test_src *tsrc, struct touch_pos *tpos)
{
	u8 rtn = 0;
	int i = 0;
...
static void mxt_patch_T57_object(struct mxt2954_ts_data *data, struct mxt2954_message *message)
{
	struct test_src tsrc = {0};
	u8 *msg = message->message;
	u8 finger_cnt = 0;
...
static void mxt_patch_T61_object(struct mxt2954_ts_data *data, struct mxt2954_message *message)
{
	int id = 0;
	u8 *msg = message->message;
	id = data->reportids[message->reportid].index;
...
static void mxt_patch_T100_object(struct mxt2954_ts_data *data, struct mxt2954_message *message)
{
	u8 id, index;
	u8 *msg = message->message;
...
{
	struct mxt2954_patch *patch_info = &data->patch;
	struct patch_header *ppheader = NULL;
	u16 stage_addr[64] = {0};
